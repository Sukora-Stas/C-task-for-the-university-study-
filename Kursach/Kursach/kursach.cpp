...
void IZadanie1() // Вывод результата в текстовый файл
{
	...
	TStudent std;
	cout << "\n|  |------------------------------------------------------------------------|  |\n";
	printf("%-3c%-3c%-25s%-10s%-10s%-6s%-9s%-10s%-1c%-1c", '|', '|', "Фамилия", "Год", "Месяц", "Кол.", "оплата", "Выплатить", '|', '\n');
	printf("%-3c%-3c%-25s%-10s%-10s%-6s%-9s%-10s%-1c%-1c", '|', '|', "сотрудника", "", "", "дней", "за день", "", '|', '\n');
	cout << "|  |------------------------------------------------------------------------|  |\n";
	while (true)
	{
		int nwrt = fread(&std, sizeof(TStudent), 1, fl);
...
void IZadanie2()
{
	...
	TStudent std;
	cout << "\n   |------------------------------------------------------------------------|\n";
	printf("%-3s%-3c%-25s%-10s%-10s%-6s%-9s%-10s%-1c%-1c", "", '|', "Фамилия", "Год", "Месяц", "Кол.", "оплата", "Выплатить", '|', '\n');
	printf("%-3s%-3c%-25s%-10s%-10s%-6s%-9s%-10s%-1c%-1c", "", '|', "сотрудника", "", "", "дней", "за день", "", '|', '\n');
	cout << "   |------------------------------------------------------------------------|\n";
	while (true)
	{
		int nwrt = fread(&std, sizeof(TStudent), 1, fl);
		...
		if (month == x){
			sum += (atoi(stud[nst].stoimost))*(atoi(stud[nst].day));
			all_sum += (atoi(stud[nst].stoimost))*(atoi(stud[nst].day));
			printf("%-3s%-3c%-25s%-10s%-10s%-6s%-3s%-7s%-5i%-4s%-1c%-1c", "", '|',
				stud[nst].FIO,
				stud[nst].year,
				month2.c_str(),
				stud[nst].day,
				...
				'|',
				'\n');
			printf("%-3s%-3c%-25s%-10s%-10s%-6s%-9s%-10s%-1c%-1c", "", '|', "", "", "", "", "", "", '|', '\n');
			sum = 0;
			nst++;
		}
	}
	cout << "   |------------------------------------------------------------------------|\n";
	printf("%-3s%-3c%-25s%-10s%-10s%-8s%-8s%-5i%-4s%-1c%-1c", "", '|', "", "", "", "", "Сумма:", all_sum, "руб", '|', '\n');
	cout << "   |------------------------------------------------------------------------|\n";
	fclose(fl);
}
