...
void Zadanie()
{
	...
	*/
	printf("%-1c%-1c%-10s%-8s%-60s%-1c%-1c", '\n', '|', "", "Задание:", "", '|', '\n');
	printf("%-1c%-5s%-8s%-17s%-1c%-1c", '|', "", "Разработка программы расчёта выплат по больничным листам", "", '|', '\n');
	printf("%-1c%-5s%-8s%-3s%-1c%-1c", '|', "", "Сведения о больничных листах сотрудников компании имеют следующий вид:", "", '|', '\n');
	printf("%-1c%-5s%-8s%-5s%-1c%-1c", '|', "", "Ф.И.О сотрудника;год; месяц; Количество дней, пропущеных по болезни;", "", '|', '\n');
	printf("%-1c%-5s%-8s%-53s%-1c%-1c", '|', "", "оплата за один день.", "", '|', '\n');
	printf("%-1c%-5s%-8s%-65s%-1c%-1c", '|', "", "", "", '|', '\n');
	printf("%-1c%-5s%-8s%-3s%-1c%-1c", '|', "", "Индивидуальное задание: для месяца Х года У вывести список сотрудников", "", '|', '\n');
	printf("%-1c%-5s%-8s%-14s%-1c%-1c", '|', "", "c указанием выплат по больничным листам для каждого из них.", "", '|', '\n');
	printf("%-1c%-5s%-8s%-4s%-1c%-1c", '|', "", "Вывести общую сумму выплат по больничнм листам за интересующий месяц.", "", '|', '\n');
	printf("%-1c%-5s%-8s%-47s%-1c%-1c", '|', "", "Х,У вводятся с клавиатуры.", "", '|', '\n');
	printf("%-1c%-5s%-8s%-65s%-1c%-1c", '|', "", "", "", '|', '\n');
	printf("%-1c%-5s%-8s%-12s%-1c%-1c", '|', "", "Общее для всех вариантов задание: реализовать авторизацию для", "", '|', '\n');
	printf("%-1c%-5s%-8s%-38s%-1c%-1c", '|', "", "входа в систему(без регистрации!), ", "", '|', '\n');
	printf("%-1c%-5s%-8s%-20s%-1c%-1c", '|', "", "функционал администратора и функционал пользователя, ", "", '|', '\n');
	printf("%-1c%-5s%-8s%-12s%-1c%-1c", '|', "", "как минимум три вида поиска, как минимум три вида сортировки.", "", '|', '\n');
}
