...
void Spisok() // Ввод данных в файла 
{
	...
	for (int i = 0; i<nst; i++)
	{
		...
		cin » stud[i].oaip;
		cout « "Vvedite otcenku po History: ";
		cin » stud[i].hist;
		cout « "Vvedite otcenku po English: ";
		cin » stud[i].english;
...
void Opf() // Открытие бинарного файла 
{
	...
	while (true)
	{
		...
		if (nwrt != 1) break;
		stud[nst] = std;
		cout « stud[nst].fio «" - Matem: " « stud[nst].matem « "; OAIP: " « stud[nst].oaip « "; History: " « stud[nst].hist « "; English: " « stud[nst].english « ";" « endl;
		nst++;
	}
	fclose(fl);
}
